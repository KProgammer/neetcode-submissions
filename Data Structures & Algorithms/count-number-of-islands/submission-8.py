    ...
    def numIslands(self, grid: List[List[str]]) -> int:
        ...
        def bfs(r,c):
            ...
            while q:
                ...
                # search the surrounding for other tiles to be visited
                ...
                for dr, dc in directions:
                    ...
                    if (r) in range(rows) and (c) in range(cols):
                        # if it is an undiscovered land point,find the island
                        if grid[r][c] == "1" and (r, c) not in visit:
                                q.append((r, c))
                                visit.add((r,c))
                        # Otherwise if it is an unexplored water tile, add it
                        # to be visited
        ...
        while to_visit:
                # print("DEBUG: to_visit",to_visit)
                row, col = to_visit.popleft()
                # print("DEBUG: row,col",row,col)
                
                # if it is an undiscovered land point,find the island
                if grid[row][col] == "1" and (row, col) not in visit:
                    # print("DEBUG: made it to first if")
                    bfs(row,col)
                    islands += 1
                else:
